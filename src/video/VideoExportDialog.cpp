    ...
    , m_defaults(defaults)
{
    ...
    m_autoZoomRadio = new QRadioButton("Automatic", mapGroup);
    m_fixedZoomRadio = new QRadioButton("Fixed zoom", mapGroup);
    m_autoZoomRadio->setChecked(defaults.autoZoom);
    m_fixedZoomRadio->setChecked(!defaults.autoZoom);
    ...
    m_fixedZoomSpin->setEnabled(!defaults.autoZoom);

    connect(m_fixedZoomRadio, &QRadioButton::toggled, m_fixedZoomSpin, &QWidget::setEnabled);

    mapLayout->addWidget(new QLabel("Style:"), 0, 0);
    mapLayout->addWidget(m_mapStyleCombo, 0, 1, 1, 2);
    mapLayout->addWidget(m_autoZoomRadio, 1, 0, 1, 3);
    mapLayout->addWidget(m_fixedZoomRadio, 2, 0);
    mapLayout->addWidget(m_fixedZoomSpin, 2, 1);
    auto* followLabel = new QLabel("Follow athlete: enabled", mapGroup);
    mapLayout->addWidget(followLabel, 3, 0, 1, 3);
...
VideoExportSettings VideoExportDialog::collectSettings() const
{
    ...
    s.autoZoom = m_autoZoomRadio->isChecked();
